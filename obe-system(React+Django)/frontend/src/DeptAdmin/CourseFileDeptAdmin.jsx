...
import '../Teacher/stylesCourseFile.css';
import Assignment from '../Teacher/CourseFileComponents/Assignment';
import ClassTest from '../Teacher/CourseFileComponents/ClassTest';
import FinalExam from '../Teacher/CourseFileComponents/FinalExam';
import { getToken} from '../services/LocalStorageService'
import { useGetLoggedUserQuery, useAddAssignmentMutation, useGetAssignmentListMutation, useAddClassTestMutation, useGetClassTestListMutation,
  useGetFinalExamInfoMutation, useSubmitCourseFileMutation, useUpdateTeacherForCFSMutation} from '../services/obesApi'

function CourseFileDeptAdmin() {
  ...
  const [updateTeacherForCFSMutation] = useUpdateTeacherForCFSMutation()
  ...
  const AddAGN= async(event) => {
    ...
    await addAssignment(actualData);
  ...
  const handleCourseFileSubmission= async(event) => {
    ...
    if (confirmation) 
    {
        ...
        else
        {
          ...
          await submitCourseFile(actualData);
          const actualData1 = {
            id: courseData.teacher_id,
          }
          await updateTeacherForCFSMutation(actualData1);
...
export default CourseFileDeptAdmin