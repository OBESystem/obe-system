...
function AssignCourse() {
  ...
  const AssignTeacher=(event) => {
    if(courseInfo[values.courseNo-1].t_id == values.teacherID)
    {
    ...
    else
    {
      ...
      .then(res =>{
        if(courseInfo[values.courseNo-1].t_id === 0)
        {
      ...
      if(courseInfo[values.courseNo-1].t_id !== 0)
      {
        axios.post('http://localhost:7000/RemoveTeacherFromCourse', { t_id: courseInfo[values.courseNo-1].t_id})
        .then(res =>{
